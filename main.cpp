...
struct AddressHasher {
    size_t operator() (const Address& address) const {
        const size_t prime = 3301;
        hash<string> str_hash;
        hash<int> int_hash;
        return  (str_hash(address.city)*prime*prime) +
                (str_hash(address.street)*prime) + int_hash(address.building);
    }
};
...
struct PersonHasher {
    size_t operator() (const Person& person) const {
        ...
        return  (str_hash(person.name)*prime*prime*prime) +
                (int_hash(person.height)*prime*prime) + (double_hash(person.weight)*prime) +
